...
namespace grade_app
{
    public partial class TeacherIndexPage : ContentPage
    {
        ...
        private async Task LoadDisciplines(long SemesterID)
        {
            ...
            if (teacherIndex.Subjects != null)
            {
                ...
                var MaxGroupNameLenght = teacherIndex.Groups.Where(kv => !GlobalDiscsIDs.Contains(kv.Key)).Select(kv => kv.Value).Max(groups =>
                {
                    static int getGroupLength(string group) => group.Length;
                    return groups.Max(getGroupLength);
                });
        ...
        private async void OnListItemTapped(object sender, ItemTappedEventArgs e)
        {
            ...
            var item = (DisciplineItemT)e.Item;
            await Navigation.PushAsync(new TeacherDisciplinePage(item.ID, [.. teacherIndex.Teachers[item.ID.ToString()].Values]));
        }
    ...
    public class DisciplineItemT(string name, long id, string groups, string type, string teachers)
    {
    ...
    public class SubjectGroup(string name, string degreeCourse) : List<DisciplineItemT>
    {
        public string Name { get; set; } = name;
